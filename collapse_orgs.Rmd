...
if(!file.exists('all_lengths.tmp.RData')){
  ...
  colnames(all_lengths) = c('version','header')
  all_lengths = all_lengths[,.(header)]
  ...
  # get species
  all_lengths[, species := sapply(header, function(x) tail(unname(unlist(strsplit(x,';'))),1))]
  save(all_lengths, file='all_lengths.tmp.RData')
}else{
...
master[, count := 1]
master[, accession := sapply(header, function(x) gsub('>ACCN:', '', unname(unlist(strsplit(x, '\\|')))[1]))]
...
# merge back with accession 
master = master[,.(accession, original_species, species)]
m = merge(master, collapsed[,.(species, new_header)], by='species')
stopifnot(nrow(m)==nrow(all_lengths))

head(m)
write.table(m[,.(accession, new_header)], 'accession_to_header_map.txt', sep='\t', col.names=F, row.names=F, quote=F)

sub = collapsed[,.(species, number_collapsed, new_header)]
sub = sub[order(species)]
head(sub)
write.table(sub, 'n_collapsed_accession_per_header.txt', sep='\t', col.names=T, row.names=F, quote=F)
```
